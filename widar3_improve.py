...
import keras
from keras.layers import AvgPool2D, SeparableConv2D, Input, GRU, Dense, Flatten, Dropout, Conv2D, Conv3D, MaxPooling2D, MaxPooling3D, TimeDistributed, BatchNormalization
...
# learning_rate_new = tf.compat.v1.train.piecewise_constant(tf.Variable(0, trainable=False), boundaries, f_learning_rates)
# # lr_schedule = tf.keras.optimizers.schedules.PiecewiseConstantDecay(
# #     boundaries, f_learning_rates, name=None
# # )
...
def assemble_model(input_shape, n_class):
    ...
    x = TimeDistributed(SeparableConv2D(16, kernel_size=(3, 3), activation='tanh', data_format='channels_last',
                                        input_shape=input_shape))(model_input)   # (@,T_MAX,20,20,1)=>(@,T_MAX,16,16,16)
    x = TimeDistributed(tf.keras.layers.AvgPool2D(pool_size=(2, 2)))(x)    # (@,T_MAX,16,16,16)=>(@,T_MAX,8,8,16)
    x = TimeDistributed(BatchNormalization())(x, training=True)
    x = TimeDistributed(SeparableConv2D(32, kernel_size=(3, 3), activation='tanh', data_format='channels_last',
                                        input_shape=input_shape))(x)   # (@,T_MAX,20,20,1)=>(@,T_MAX,16,16,16)
    x = TimeDistributed(tf.keras.layers.AvgPool2D(pool_size=(2, 2)))(x)    # (@,T_MAX,16,16,16)=>(@,T_MAX,8,8,16)
    x = TimeDistributed(Flatten())(x)   # (@,T_MAX,8,8,16)=>(@,T_MAX,8*8*16)
    x = TimeDistributed(Dense(128, activation='tanh'))(x)  # (@,T_MAX,8*8*16)=>(@,T_MAX,64)
    x = TimeDistributed(Dropout(f_dropout_ratio))(x)
    x = TimeDistributed(Dense(64, activation='tanh'))(x)  # (@,T_MAX,64)=>(@,T_MAX,64)
